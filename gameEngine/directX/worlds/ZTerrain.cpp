...
HRESULT	ZTerrain::_CreateVIB()
{
	...
	if( FAILED( m_pd3dDevice->CreateVertexBuffer( m_cxDIB*m_czDIB*sizeof(TERRAINVERTEX),
												  D3DUSAGE_DYNAMIC, TERRAINVERTEX::FVF, D3DPOOL_SYSTEMMEM, &m_pVB, NULL ) ) )
    {
        _Destroy();
		return E_FAIL;
    }
...
#ifdef _USE_INDEX16
	// 生成IB
    if( FAILED( m_pd3dDevice->CreateIndexBuffer( (m_cxDIB-1)*(m_czDIB-1)*2 * sizeof(TRIINDEX), D3DUSAGE_DYNAMIC, D3DFMT_INDEX16, D3DPOOL_SYSTEMMEM, &m_pIB, NULL ) ) )
    {
        _Destroy();
        return E_FAIL;
    }
...
#else
	// 生成IB
    if( FAILED( m_pd3dDevice->CreateIndexBuffer( (m_cxDIB-1)*(m_czDIB-1)*2 * sizeof(TRIINDEX), D3DUSAGE_DYNAMIC, D3DFMT_INDEX32, D3DPOOL_SYSTEMMEM, &m_pIB, NULL ) ) )
    {
        _Destroy();
        return E_FAIL;
    }
...
HRESULT	ZTerrain::_Render()
{
	...
	m_pd3dDevice->SetSamplerState( 0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR );	// 0号纹理层的放大滤镜
...
/// 地形输出到画面.
HRESULT	ZTerrain::Draw(ZFrustum* g_pFrustum )
{
	LPDWORD		pI;

    if( FAILED( m_pIB->Lock( 0, (m_cxDIB-1)*(m_czDIB-1)*2 * sizeof(TRIINDEX), (void**)&pI, 0 ) ) )
        return E_FAIL;
	m_nTriangles = m_pQuadTree->GenerateIndex( pI,m_pvHeightMap,g_pFrustum, 0.1);
    m_pIB->Unlock();
